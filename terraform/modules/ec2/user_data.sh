...
if docker exec ftp-server pure-pw show "${ftp_username}" >/dev/null 2>&1; then
  echo "User ${ftp_username} already exists, removing..." >> /var/log/ftp-setup.log
  docker exec ftp-server pure-pw userdel "${ftp_username}" || true
fi

echo "Creating FTP user ${ftp_username}..." >> /var/log/ftp-setup.log
# Create user with explicit password input
printf "%s\n%s\n" "${ftp_password}" "${ftp_password}" | docker exec -i ftp-server \
  pure-pw useradd "${ftp_username}" -u ftpuser -g ftpuser -d "/home/ftpusers/${ftp_username}" -m

echo "Building user database..." >> /var/log/ftp-setup.log
docker exec ftp-server pure-pw mkdb

echo "Verifying user creation..." >> /var/log/ftp-setup.log
...
cat > /etc/systemd/system/ftp-s3-sync.service << 'SYSTEMD_UNIT_EOF'
[Unit]
...
echo "Restarting FTP container to load user database..." >> /var/log/ftp-setup.log
